...
class AppState(BaseModel):
    ...
    order_value_pct_balance: float = 0.01
    max_leverage: int = 5
    max_positions: int = 2
    max_daily_loss_pct: float = 0.02
    cooldown_seconds: int = 120
    max_trades_per_hour: int = 10
    spread_guard_bps: float = 3.0
    max_slippage_bps: float = 12.0
    edge_gate_factor: float = 0.65
    vol_10s_threshold: float = 0.006
    regime_filter_enabled: bool = True
    trend_strength_threshold: float = 0.25
    time_stop_seconds: int = 120
    tp_profile: list[float] = Field(default_factory=lambda: [0.3, 0.5, 0.6])
    impulse_threshold_pct: float = 3.0
    impulse_window_seconds: int = 60
    exhaustion_ratio_threshold: float = 0.4
